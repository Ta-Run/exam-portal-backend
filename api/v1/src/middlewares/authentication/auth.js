...
async function authentication (req, res, next) {
  ...
  if(decodedJwtToken.loginType == "Admin"){
    ...
    const isRevoked = await adminAccessToken.findOne({adminId:user.id, jti:{$in: decodedJwtToken.jti}})
    if (!user) {
      return res.json({
  ...
  // Client auth
    ...
    const user =await ClientModel.findOne({'clientEmail':decodedJwtToken.email})
    ...
    const isRevoked = await clientAccessTokenModel.findOne({clientId:user.id, jti:{$in: decodedJwtToken.jti}})
    if( isRevoked.revoked === true){
      return res.json({
        message: "unauthorized",
      });
  ...
  // Spoc Person auth
    ...
    const user =await SpocPersonModel.findOne({'emailId':decodedJwtToken.email})
    const isRevoked = await spocPersonAccessTokenModel.findOne({spocPersonId:user.id, jti:{$in: decodedJwtToken.jti}})
    if( isRevoked.revoked === true){
      return res.json({
        message: "unauthorized",
      });
  ...
  // Child User auth
    ...
    const user =await ChildUserModel.findOne({'emailId':decodedJwtToken.email})
    const isRevoked = await ChildUserAccessTokenModel.findOne({chlildUserId:user._id,jti:{$in:decodedJwtToken.jti}})
    if(isRevoked.revoked === true){
      return res.json({
        message: "unauthorized",
      });
