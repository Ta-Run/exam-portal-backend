...
//Sector Router
router.post('/sector-add', auth, singleFileUpload("logo"), sectorController.sectorCreate)
router.get('/sector', auth, sectorController.sectorList)
router.put('/sector/:id', singleFileUpload("logo"), auth, sectorController.sectorEdit)
...
router.put('/spoc-person/status/:id', auth, spocpersonController.spocPersonStatusChange)
router.delete('/spoc-person/remove/:id', auth, spocpersonController.spocPersonRemove)
...
router.post("/nos/bulkUpload",auth, singleFileUpload("bulkUpload"),nosController.bulkUpload)
...
router.delete("/question-bank/remove/:id", auth, questionBankController.questionBankRemove)
router.put("/question-bank/edit/:id", auth, questionBankController.questionBankEdit)
router.put("/question-bank/status/:id", auth, questionBankController.questionBankStatus)

//Question Router
router.post("/question/add", auth, multiFieldsUpload(
  [
...
router.put("/question-status/:id",auth,questionController.questionStatusChange)
router.put("/question-edit/:id", auth, multiFieldsUpload(
  [
    { name: 'attatchment', maxCount: 1 },
    { name: 'optionAAttatchment', maxCount: 1 },
    { name: 'optionBAttatchment', maxCount: 1 },
  ...
  //Question BulkUpload
  router.post("/question/bulkUpload", singleFileUpload("bulkUpload"),auth, questionController.questionBulkUpload)
  router.get("/bulk-upload/question",auth, questionController.getQuestionBulkUpload)
...
router.put("/child-user/status/:id",auth,childUserController.childUserStatusChange)
router.delete("/child-user/remove/:id",auth,childUserController.childUserRemove)

// Assessor Route 
router.post("/assessor/register",auth,multiFieldsUpload( [
...
router.put("/assessor/:id",auth,multiFieldsUpload( [
  { name: 'profile_picture', maxCount: 1 },
  { name: 'adhar_img', maxCount: 1 },
  { name: 'adhar_img2', maxCount: 1 },
  { name: 'pancard_img', maxCount: 1 }
]),assessorController.assEdit)
router.put("/assessor/update-status/:id",auth,assessorController.statusUpdate)
router.delete("/assessor/delete-ass/:id",auth,assessorController.assDelete)


//add Candidate
router.post("/candidate/add",auth,singleFileUpload("ProfilePicture"),candidateController.addCandidate)
router.get("/candidate/list",auth,candidateController.candidateList)
router.post("/candidate/bulkUpload",auth,singleFileUpload("bulkUpload"),candidateController.bulkupload)

//manage Candidates
router.post("/manageCandidate/move",auth,manageCandidateController.moveCandidate)
router.post("/manageCandidate/delete", auth,manageCandidateController.deleteCandidate);
router.get("/manageCandidate/list", auth,manageCandidateController.manageCandidateList);
router.put("/manageCandidate/Edit/:id", auth,singleFileUpload("ProfilePicture"),manageCandidateController.Edit);
router.delete("/manageCandidate/delete/:id", auth,manageCandidateController.deletebyId);

//Assessment -Batch Upload
router.post("/assessment/bulk_upload",auth,singleFileUpload("bulkUpload"),assessmentController.batch_bulkupload)
router.post("/assessment/moveBulk",auth,assessmentController.moveBatch)  //move to manage bulk 
router.post("/assessment/deleteBulkData",auth,assessmentController.deleteBatch)
...
router.put("/assessment/editBatchData/:id",auth,assessmentController.editBatchData)
router.delete("/assessment/deleteBatchData/:id",auth,assessmentController.deleteBatchData)
router.get("/manageCandidate/scheduleBatch", auth,assessmentController.scheduleBatchList);
router.get("/manageCandidate/currentBatch", auth,assessmentController.currentBatch);
router.get("/manageCandidate/batch/dropDown",auth,assessmentController.dropDown);

//application 
router.post("/application/upload",auth,multiFieldsUpload( [
...
router.get("/spoc-person/logout",auth,SpocPersonAuthController.spocPersonLogout)
router.get("/spoc-person/assign-job-role",auth,SpocPersonAuthController.assignJobRole)
